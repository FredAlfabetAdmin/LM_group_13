...
c_createPrimitiveShape        = ct.CFUNCTYPE(ct.c_int32,ct.c_int32,ct.POINTER(ct.c_char),ct.c_int32,ct.POINTER(ct.c_char),ct.c_int32,ct.POINTER(ct.c_int32),ct.c_int32,ct.POINTER(ct.c_float),ct.c_int32,ct.POINTER(ct.c_char),ct.c_int32,ct.POINTER(ct.c_ubyte),ct.POINTER(ct.c_int32), ct.POINTER(ct.POINTER(ct.c_int32)),ct.POINTER(ct.c_int32), ct.POINTER(ct.POINTER(ct.c_float)),ct.POINTER(ct.c_int32), ct.POINTER(ct.POINTER(ct.c_char)),ct.POINTER(ct.c_int32), ct.POINTER(ct.POINTER(ct.c_ubyte)),ct.c_int32)(("simxCallScriptFunction", libsimx))
c_createPrimitiveShape
# fmt: on
...
def simxGetJointMaxForce(clientID, jointHandle, operationMode):
    ...
    force = ct.c_float()
    return (
        c_GetJointMaxForce(clientID, jointHandle, ct.byref(force), operationMode),
        force.value,
    )
...
def simxSetObjectQuaternion(
    ...
    return c_SetObjectQuaternion(
        clientID, objectHandle, relativeToObjectHandle, quat, operationMode
    )
...
def simxGetInt32Param(clientID, paramIdentifier, operationMode):
    ...
    paramValue = ct.c_int()
    return (
        c_GetInt32Param(clientID, paramIdentifier, ct.byref(paramValue), operationMode),
        paramValue.value,
    )
...
def simxGetFloatParam(clientID, paramIdentifier, operationMode):
    ...
    return (
        c_GetFloatParam(clientID, paramIdentifier, ct.byref(paramValue), operationMode),
        paramValue.value,
    )
...
def simxCheckCollision(clientID, entity1, entity2, operationMode):
    ...
    """
    collisionState = ct.c_ubyte()
    return c_CheckCollision(
        clientID, entity1, entity2, ct.byref(collisionState), operationMode
    ), bool(collisionState.value != 0)
...
def simxCheckDistance(clientID, entity1, entity2, operationMode):
    ...
    return (
        c_CheckDistance(
            clientID, entity1, entity2, ct.byref(minimumDistance), operationMode
        ),
        minimumDistance.value,
    )
...
def simxGetInt32Signal(clientID, signalName, operationMode):
    ...
    if (sys.version_info[0] == 3) and (type(signalName) is str):
        signalName = signalName.encode("utf-8")
    return (
        c_GetInt32Signal(clientID, signalName, ct.byref(signalValue), operationMode),
        signalValue.value,
    )
...
def simxSetObjectFloatParameter(
    ...
    return simxSetObjectFloatParam(
        clientID, objectHandle, parameterID, parameterValue, operationMode
    )


@simx_function
def simxSetObjectFloatParam(
    clientID, objectHandle, parameterID, parameterValue, operationMode
):
    """
    Please have a look at the function description/documentation in the CoppeliaSim user manual
...
def simxSetObjectIntParameter(
    ...
    return simxSetObjectInt32Param(
        clientID, objectHandle, parameterID, parameterValue, operationMode
    )


@simx_function
def simxSetObjectInt32Param(
    clientID, objectHandle, parameterID, parameterValue, operationMode
):
    """
    Please have a look at the function description/documentation in the CoppeliaSim user manual
...
#createPrimitiveShape(int primitiveType, list sizes, int options = 0)
@simx_function
def simxcreatePrimitiveShape(clientID, objectHandle, operationMode):
    ret = c_createPrimitiveShape(
