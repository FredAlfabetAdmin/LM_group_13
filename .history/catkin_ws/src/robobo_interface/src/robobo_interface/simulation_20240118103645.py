...
class SimulationRobobo(IRobobo):
    ...
    def _initialise_handles(self) -> None:
        ...
        for kid in kids:
            ...
            x_random_position = random.randint(x_range_min*100, x_range_max*100, 1)
            y_random_position = random.randint(y_range_min*100, y_range_max*100, 1) / 100
            x_random_position = x_random_position / 100
            y_random_position = y_random_position / 100
            print("in kid")
            z_pos = self._target.position['z']
            print(f"z_pos: {z_pos}")
            sim.simxSetObjectPosition(self._connection_id, kid, False, [x_random_position, y_random_position,1], sim.simx_opmode_blocking)
            print("random tp")
        ...
        #data = sim.children(self._connection_id, handle_dummy, sim.simx_opmode_blocking)
        # Possibly getting all the children by split
        # '/Danger[0]/joint',{index=0}

        # createPrimitiveShape to dynamically create the objects. Isn't supported right now.
        #shapeHandle = sim.createPrimitiveShape(primitiveType

        # To retreive the children:
        #simxGetObjectHandle(self._connection_id, '/Dummy_Danger', simConst.simx_opmode_blocking)
        # sim.getObjectHandle('/Dummy_Danger') # In coppeliaSim
