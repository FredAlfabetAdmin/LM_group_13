...
class SimulationRobobo(IRobobo):
    ...
    def _initialise_handles(self) -> None:
        ...
        print("hi")
        test = sim.simxCreateBuffer(1)
        print("sphere")
        sphere_handle = sim.simxCreateSphere(self._connection_id, sphere_size, None, position, sim.simx_opmode_blocking)
        sim.simxCreateSphere()
int shapeHandle = sim.createPrimitiveShape(int primitiveType, list sizes, int options = 0)
        print("done")
        objects = []
        print("hoi")
        
        generated_object = copy.deepcopy(self._target)
        print("copied")
        generated_object.set_position('/SceneObject',sim.handle_world, [1,1,1])
        print("Fin")