...
class SimulationRobobo(IRobobo):
    ...
    def _initialise_handles(self) -> None:
        ...
        handle_dummy = sim.getObjectHandle(self._connection_id, '/Dummy_Danger', simConst.simx_opmode_blocking)
        print(handle_dummy)
        data = sim.simxGetObjects(self._connection_id, handle_dummy, sim.simx_opmode_blocking)
        print(data)
        for dp in data:
            print(dp)
